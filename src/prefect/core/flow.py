...
class Flow:
    ...
    def cli(self) -> None:
        # import fire
        # class FlowCLI(object):
        #     run = lambda s: self.run()
        #     save = self.save
        #     load = self.load
        #     serialize = self.serialize
        #     register = self.register
        # return fire.Fire(FlowCLI, name='flow')
        ...
        self.result_handler = LocalResultHandler(dir=".prefect/results")

        @click.group()
        def main():
            pass

        @main.command(help="run your flow")
        def run():
            state = self.run()
            self.save(fpath=os.path.abspath(".prefect/flow"))
            with open(str(".prefect/state"), "wb") as f:
                cloudpickle.dump(state, f)

        @main.command(help="show results from your flow")
        def results():
            state = None
        ...
        @main.command(help="register your flow with cloud")
        @click.option(
            "--project",
            required=True,
        ...
        def register(project):
            flow_id = self.register(project_name=project)
            with open(str(".prefect/registration"), "wb") as f:
                cloudpickle.dump(flow_id, f)
            click.echo(flow_id)
            return flow_id

        @main.command(help="run your flow in Prefect Cloud")
        @click.option(
            "--project",
            default=None,
            help="the Prefect Cloud project to register your flow with",
